...
class FailedDevicePage extends StatefulWidget {
  const FailedDevicePage({super.key});

  @override
  State<FailedDevicePage> createState() => _FailedDevicePageState();
}

class _FailedDevicePageState extends State<FailedDevicePage> {
    ...
    return RefreshIndicator(
      onRefresh: () async {
        overlayService.showSuccessNotification(
          (context) => "Refreshed failed devices list",
          duration: const Duration(milliseconds: 2000),
        );
      },
      child: BlocBuilder<DeviceBloc, DeviceState>(
        builder: (context, state) {
          final failedDevices = state.failedDevices;
          ...
          return Column(
            children: [
              ...
              Row(
                ...
                children: [
                  buildClearButton(context, failedDevices),
                  const SizedBox(width: 6),
                  buildExportButton(context, failedDevices),
                ],
              ),
              Expanded(
                child: ListView.separated(
                  ...
                  itemBuilder: (context, index) {
                    final device = failedDevices[index];
                    return ListTile(
                      dense: true,
                      ...
                      leading: Column(
                        ...
                        children: [
                          Text(
                            '${device.message} | ${device.hardware}',
                            style: const TextStyle(
                              fontSize: 11,
                          ...
                          Text(
                            '${device.major}-${device.minor} | ${device.txPower} | ${device.interval == 0 ? "Default" : "${device.interval} ms"}',
                            style: const TextStyle(
                              fontSize: 11,
                      ...
                      trailing: IconButton(
                        ...
                        onPressed: () async {
                          ...
                          if (confirmed == true) {
                            deviceBloc.add(RemoveFailedDevice(device));
  ...
  Widget buildClearButton(
    ...
    return ElevatedButton.icon(
      onPressed: () async {
        ...
        bool? confirmed = await showDialog<bool>(
          ...
          builder:
              (_) => AlertDialog(
                ...
                content: const Text(
                  "Are you sure you want to clear all failed devices?",
                ),
                actions: [
                  TextButton(
                    onPressed:
        ...
        if (confirmed == true) {
          deviceBloc.add(ClearFailedDevices());
          overlayService.showSuccessNotification(
            (context) => "All failed devices cleared",
            duration: const Duration(milliseconds: 2000),
          );
        }
      },
  ...
  Widget buildExportButton(
    ...
    return ElevatedButton.icon(
      onPressed: () async {
        if (failedDevices.isEmpty) {
          overlayService.showSuccessNotification(
            (context) => "No failed devices available for export",
            duration: const Duration(milliseconds: 2000),
          );
          return;
        }
        ...
        bool? confirmed = await showDialog<bool>(
          ...
          builder:
              (_) => AlertDialog(
                ...
                content: const Text(
                  "Are you sure you want to export all failed devices?",
                ),
                actions: [
                  TextButton(
                    onPressed:
        ...
        // 开始导出
        String csv =
            "Name,MAC,MAJOR,MINOR,TX Power,Interval(ms),Hardware,Message\n";
        for (var d in failedDevices) {
          csv +=
              "${d.name},${d.id},${d.major},${d.minor},${d.txPower},${d.interval},${d.hardware},${d.message}\n";
        }
        ...
        try {
          ...
          File file = File(
            "${downloadsDir.path}/failed_devices_$timestamp.csv",
          );
